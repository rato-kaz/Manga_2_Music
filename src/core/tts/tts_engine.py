...
class StyleBertVITS2Wrapper:
    ...
    def generate(
        ...
        logger.info(
            f"Generating speech for character {voice_profile.character_id}: "
            f"{text[:30]}..."
        )
    ...
    def load_profiles(self) -> None:
        ...
        if self.profiles_file.exists():
            try:
                ...
                logger.warning(f"Error loading voice profiles: {e}", exc_info=True)
...
def generate_speech_for_bubble(
    ...
    speed, pitch = adjust_prosody_for_emotion(
        voice_profile.speed,
        voice_profile.pitch,
        emotion or "neutral",
    )
