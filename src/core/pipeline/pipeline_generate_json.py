...
# -----------------------------------------------------------------------------
# Optional OpenAI VLM integration
# -----------------------------------------------------------------------------
        ...
        if describe_panel:
            for panel, panel_bbox in zip(panel_entries, panel_bboxes):
                ...
                try:
                    ...
                    logger.warning(f"VLM description failed for {page.path}: {exc}", exc_info=True)
                    description = None
                if description:
...
def build_output_json(
    ...
    *,
    ...
    describe_panel = build_openai_describer(use_vlm)
    ...
    logger.info(f"Saved {len(all_entries)} pages to {output_path}")
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--use-vlm",
        action="store_true",
        help="If set, use OpenAI VLM to add panel descriptions (requires OPENAI_BASE_URL, OPENAI_API_KEY, OPENAI_MODEL in .env).",
    )
    parser.add_argument(
        "--no-enhancements",
        action="store_true",
        help="Disable enhanced features (reading order, character re-ID, manpu, etc.).",
    )
